...
//		}
		...
		BreakerConfig config = dao.getConfig(137);
/*		TimeZone tz = TimeZone.getTimeZone("America/Chicago");
		...
		ResourceLoader.writeFile("d:\\zwave\\breakerConfig137.json", DaoSerializer.toJson(config));
		config.getAllBreakers().forEach(_b->_b.setPolarity(BreakerPolarity.BI_DIRECTIONAL));
		CollectionUtils.getFirst(config.getBreakerHubs()).setPhaseOffsetNs(13333333);
		config.getAllBreakers().stream().filter(_b->_b.getName().contains("L2")).forEach(_b->_b.setPhaseOffsetNs(6666667));
		config.getAllBreakers().stream().filter(_b->_b.getName().contains("L3")).forEach(_b->_b.setPhaseOffsetNs(13333333));
